...
// is thrown.
export async function validateFileDigest(filePath, expectedDigest) {
...
export async function downloadSLSAVerifier(version, digest) {
    core.debug(`Downloading slsa-verifier ${version}`);
    // Download the slsa-verifier binary.
    const verifierPath = await tc.downloadTool(`https://github.com/slsa-framework/slsa-verifier/releases/download/${version}/slsa-verifier-linux-amd64`);
...
// provenance.
export async function downloadAndVerifySLSA(url, provenanceURL, sourceURI, sourceTag, slsaVerifierVersion, slsaVerifierDigest) {
    ...
    const { exitCode, stdout, stderr } = await exec.getExecOutput(verifierPath, [
        "verify-artifact",
        artifactPath,
        "--provenance-path",
        provenancePath,
